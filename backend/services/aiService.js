...
const { generateFallbackMermaidGraph } = require("../utils/mermaidFallback");
...
function generateFallbackMermaidGraph(studyPlan = {}, metadata = {}) {
  ...
  // Build safe node IDs and labels
  let lines = [];
  lines.push('```mermaid');
  lines.push('graph TD');
...
async function generateMermaidStudyGraph(studyPlan = {}, metadata = {}) {
  try {
    console.log("Generating Mermaid graph for plan...");
    
    // Always use fallback - it's simpler and more reliable
    const graph = generateFallbackMermaidGraph(studyPlan, metadata);
    console.log("Fallback graph generated, length:", graph.length);
    return graph;
    
  } catch (error) {
    console.error("Mermaid graph generation error:", error);
    return generateFallbackMermaidGraph(studyPlan, metadata);
  }
}
